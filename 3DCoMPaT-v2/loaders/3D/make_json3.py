...
# メインの画像フォルダのパス
unstable_image_folder = "/home/itoh/ClipMesh/3DCoMPaT-v2/loaders/3D/scene_all_unstable"
...
# 画像ファイルごとにデータを作成
for image_file in sorted(os.listdir(unstable_image_folder), key=lambda x: int(os.path.splitext(x)[0])):
    # 各画像ファイルに対してループ

    image_path = os.path.join(unstable_image_folder, image_file)
    # 画像のフルパスを取得
    ...
    for template_func in openai_imagenet_template:
        generated_caption = template_func("unstable chair")
...
# JSONファイルに書き込む
with open('images_template.json', 'w') as json_file:
